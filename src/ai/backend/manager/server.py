...
VALID_VERSIONS: Final = frozenset(
    [
        ...
        # added "filter" and "order" arg to all paginated GQL queries
        # with their own expression mini-langs
...
async def exception_middleware(
    ...
    try:
        ...
        await stats_monitor.report_metric(
            INCREMENT,
            f"ai.backend.manager.api.status.{ex.status_code}",
        )
        raise
        ...
        await stats_monitor.report_metric(
            INCREMENT,
            f"ai.backend.manager.api.status.{resp.status}",
        )
        return resp
...
async def webapp_plugin_ctx(root_app: web.Application) -> AsyncIterator[None]:
    ...
    for plugin_name, plugin_instance in plugin_ctx.plugins.items():
        ...
        subapp, global_middlewares = await plugin_instance.create_app(
...
def handle_loop_error(
    ...
    if exception is not None:
        if sys.exc_info()[0] is not None:
            ...
            if (error_monitor := getattr(root_ctx, "error_monitor", None)) is not None:
                loop.create_task(
                    error_monitor.capture_exception(exc_instance=exception),
                )
    ...
    *,
    ...
    async def _cleanup_context_wrapper(
        ...
        try:
            ...
            log.error(
                "Error initializing cleanup_contexts: {0}",
                cctx.__name__,
                exc_info=exc_info,
            )
