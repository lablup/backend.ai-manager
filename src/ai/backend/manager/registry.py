...
if TYPE_CHECKING:
    ...
    from .scheduler.types import PendingSession, SchedulingContext
        ...
        def __getattr__(self, name: str):
            if f := self._cached_funcs.get(name, None):
                ...
                async def _wrapped(*args, **kwargs):
                    ...
                    ret = await self.peer.invoke(
                        name, request_body, order_key=self.order_key.get()
                    )
                    self.peer.last_used = time.monotonic()
                    return ret
...
class AgentRegistry:
    ...
    async def update_instance(self, inst_id, updated_fields):
        async with self.db.begin() as conn:
            query = (
                sa.update(agents).values(**updated_fields).where(agents.c.id == inst_id)
            )
            await conn.execute(query)

    async def gather_agent_hwinfo(
        self, instance_id: AgentId
    ...
    async def get_kernels(
        ...
        *,
        ...
        cond_id = (
            sa.sql.expression.cast(kernels.c.id, sa.String).like(
        ...
        cond_session_id = (
            sa.sql.expression.cast(kernels.c.session_id, sa.String).like(
                f"{session_name_or_id}%"
            )
        ) & (kernels.c.access_key == access_key)
    ...
    async def enqueue_session(
        ...
        *,
        ...
        async with self.db.begin() as conn:
            ...
            for item in matched_vfolders:
                if item["group"] is not None and item["group"] != str(group_id):
                    ...
                    continue
                mount_path = await self.storage_manager.get_mount_path(
                    item["host"], item["id"]
                )
                if item["name"] == ".local" and item["group"] is not None:
                    try:
                        async with self.storage_manager.request(
                            ...
                            "folder/file/mkdir",
                            params={
                                "volume": self.storage_manager.split_host(item["host"])[
                                    1
                                ],
                                "vfid": item["id"],
        ...
        if is_multicontainer:
            if len(kernel_enqueue_configs) == 1:
                ...
                for i in range(cluster_size - 1):
                    sub_kernel_config = cast(
                        KernelEnqueueingConfig, {**kernel_enqueue_configs[0]}
                    )
        ...
        for kernel in kernel_enqueue_configs:
            ...
            parse_service_ports(
                image_info["labels"].get("ai.backend.service-ports", ""), BackendError
            )
            ...
            if shmem is None:
                shmem = image_info["labels"].get(
                    "ai.backend.resource.preferred.shmem", "64m"
                )
            ...
            if "resources" in creation_config:
                ...
                if gpu is not None:
                    raise InvalidAPIParameters(
                        "Client upgrade required " "to use GPUs (v19.03+)."
                    )
                tpu = creation_config.get("instanceTPUs")
                if tpu is not None:
                    raise InvalidAPIParameters(
                        "Client upgrade required " "to use TPUs (v19.03+)."
                    )
            ...
            if not (requested_slots <= image_max_slots):
                raise InvalidAPIParameters(
                    ...
                    "the maximum allowed by the image. ({})".format(
                        " ".join(
                            f"{k}={v}"
                            for k, v in image_max_slots.to_humanized(
                                known_slot_types
                            ).items()
                        )
                    )
                    ...
                    if dotfile["path"].startswith("/"):
                        if dotfile["path"].startswith("/home/"):
                            ...
                            if (
                                len(path_arr) >= 3
                                and path_arr[2] == "work"
                                and path_arr[3] in matched_mounts
                            ):
                                raise BackendError(
                ...
                query = kernels.insert().values(
                    {
                        ...
                        "mounts": [
                            list(mount) for mount in mounts
                        ],  # postgres save tuple as str
                        "mount_map": mount_map,
    ...
    async def start_session(
        ...
        async with self.db.begin() as conn:
            query = (
                ...
                .where(
                    ...
                    == scheduled_session.resource_policy
                )
            )
            result = await conn.execute(query)
        ...
        if scheduled_session.cluster_mode == ClusterMode.SINGLE_NODE:
            if scheduled_session.cluster_size > 1:
                ...
                try:
                    ...
                    log.exception(
                        f"Failed to create an agent-local network {network_name}"
                    )
                    raise
            else:
                network_name = None
        ...
        scheduled_session.environ.update(
            {
                ...
                "BACKENDAI_CLUSTER_REPLICAS": ",".join(
                    f"{k}:{v}" for k, v in replicas.items()
                ),
                        ...
                        .values(
                            {
                                ...
                                "occupied_shares": {},
                                "attached_devices": created_info.get(
                                    "attached_devices", {}
                                ),
                ...
                async with RPCContext(
                    ...
                    try:
                            ...
                            [
                                {
                                    "image": {
                                        ...
                                        "digest": image_infos[binding.kernel.image_ref][
                                            "digest"
                                        ],
                                        "repo_digest": None,
                                        "canonical": binding.kernel.image_ref.canonical,
                                        "labels": image_infos[binding.kernel.image_ref][
                                            "labels"
                                        ],
                                    },
                                    ...
                                    "environ": {
                                        **scheduled_session.environ,
                                        "BACKENDAI_KERNEL_ID": str(
                                            binding.kernel.kernel_id
                                        ),
                                        "BACKENDAI_KERNEL_IMAGE": str(
                                            binding.kernel.image_ref
                                        ),
                                        "BACKENDAI_CLUSTER_ROLE": binding.kernel.cluster_role,
                                        "BACKENDAI_CLUSTER_IDX": str(
                                            binding.kernel.cluster_idx
                                        ),
                                        "BACKENDAI_CLUSTER_HOST": str(
                                            binding.kernel.cluster_hostname
                                        ),
                                    },
                        ...
                        try:
                            ...
                            for binding in items:
                                self._post_kernel_creation_tasks.pop(
                                    binding.kernel.kernel_id, None
                                )
                    except Exception:
        ...
        if per_agent_tasks:
            ...
            for agent_alloc_tx, result in zip(
                (item[0] for item in per_agent_tasks), results
            ):
    ...
    async def get_keypair_occupancy(self, access_key, *, conn=None):
        ...
        async with reenter_txn(self.db, conn) as conn:
            ...
            zero = ResourceSlot()
            key_occupied = sum(
                [row["occupied_slots"] async for row in (await conn.stream(query))],
                zero,
            )
        ...
        async with self.db.begin() as conn:
            ...
            if len(concurrency_used_per_key) > 0:
                ...
                query = (
                    ...
                    .where(
                        sa.not_(
                            keypairs.c.access_key.in_(concurrency_used_per_key.keys())
                        )
                    )
                )
                await conn.execute(query)
    ...
    async def destroy_session_lowlevel(
        ...
        for agent_id, group_iterator in itertools.groupby(
            ...
            for kernel in grouped_kernels:
                if (
                    kernel["container_id"] is not None
                    and kernel["agent_addr"] is not None
                ):
                ...
                for kernel in grouped_kernels:
                    if kernel["status"] == KernelStatus.PENDING:
                        ...
                        raise GenericForbidden(
                            "Cannot destroy kernels in pulling status"
                        )
                        ...
                        async with self.db.begin() as conn:
                            if kernel["cluster_role"] == DEFAULT_ROLE:
                                ...
                                await conn.execute(
                                    ...
                                    .where(
                                        keypairs.c.access_key == kernel["access_key"]
                                    )
                                )
                            await conn.execute(
                                sa.update(kernels)
                            ...
                            await conn.execute(
                                sa.delete(vfolder_attachment).where(
                                    vfolder_attachment.c.kernel == kernel["id"]
                                )
                            )
                            await self.event_producer.produce_event(
                    ...
                    if kernel["agent_addr"] is None:
                        await self.mark_kernel_terminated(
                            kernel["id"], "missing-agent-allocation"
                        )
                        if kernel["cluster_role"] == DEFAULT_ROLE:
                        ...
                        for kernel in destroyed_kernels:
                            ...
                            try:
                                raw_last_stat = await redis.execute_with_retries(
                                    lambda: self.redis_stat.get(
                                        str(kernel["id"]), encoding=None
                                    ),
                                    max_retries=3,
                                )
    ...
    async def clean_session(
        ...
        async with self.db.begin() as conn:
            ...
            if session is None:
                return
        if (
            session["cluster_mode"] == ClusterMode.SINGLE_NODE
            and session["cluster_size"] > 1
        ):
            ...
            try:
                ...
                log.exception(
                    f"Failed to destroy the agent-local network {network_name}"
                )
        ...
        async def _restart_kernel(kernel) -> None:
            try:
                ...
                try:
                    ...
                    del self.kernel_creation_tracker[(kernel_creation_id, kernel["id"])]
            except Exception:
    ...
    async def execute_batch(
        ...
        async with self.db.begin() as conn:
            ...
            async with RPCContext(
                ...
                return await rpc.call.execute_batch(
                    str(kernel["id"]), kernel["startup_command"]
                )
    ...
    async def shutdown_service(
        ...
        kernel = await self.get_session(session_name_or_id, access_key)
        async with self.handle_kernel_exception(
            "shutdown_service", kernel["id"], access_key
        ):
            async with RPCContext(
                kernel["agent"],
                kernel["agent_addr"],
    ...
    async def upload_file(
        ...
        kernel = await self.get_session(session_name_or_id, access_key)
        async with self.handle_kernel_exception(
            "upload_file", kernel["id"], access_key
        ):
            async with RPCContext(
                kernel["agent"],
                kernel["agent_addr"],
    ...
    async def download_file(
        ...
        kernel = await self.get_session(session_name_or_id, access_key)
        async with self.handle_kernel_exception(
            "download_file", kernel["id"], access_key
        ):
            async with RPCContext(
                kernel["agent"],
                kernel["agent_addr"],
    ...
    async def get_logs_from_agent(
        ...
        kernel = await self.get_session(session_name_or_id, access_key)
        async with self.handle_kernel_exception(
            "get_logs_from_agent", kernel["id"], access_key
        ):
            async with RPCContext(
                kernel["agent"],
                kernel["agent_addr"],
    ...
    async def kill_all_sessions(self, conn=None):
        async with reenter_txn(self.db, conn) as conn:
            ...
            tasks = []
            for row in rows:
                tasks.append(self.kill_all_sessions_in_agent(row["id"], row["addr"]))
    ...
    async def handle_heartbeat(self, agent_id, agent_info):
        ...
        slot_key_and_units = {
            ...
            for k, v in agent_info["resource_slots"].items()
        }
        available_slots = ResourceSlot(
            {SlotName(k): v[1] for k, v in agent_info["resource_slots"].items()}
        )
        ...
        async with self.heartbeat_lock:
            ...
            async with self.db.begin() as conn:
                ...
                if row is None or row["status"] is None:
                    ...
                    if updates:
                        await self.shared_config.update_resource_slots(
                            slot_key_and_units
                        )
                        query = (
                            sa.update(agents)
        ...
        *,
                        ...
                        updates.update(
                            {
                                ...
                                "io_max_scratch_size": int(
                                    kern_stat["io_max_scratch_size"]
                                ),
                            }
                        )
        ...
        async with reenter_txn(self.db, db_conn) as conn:
            for kernel_id, updates in per_kernel_updates.items():
                query = (
                    sa.update(kernels).values(updates).where(kernels.c.id == kernel_id)
                )
                await conn.execute(query)
