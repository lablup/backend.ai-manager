                ...
                if len(concurrency_used_per_key) > 0:
                    ...
                    for ak, used in concurrency_used_per_key.items():
                        query = (
                            ...
                            .where(keypair_resource_usages.c.access_key == ak)
                        )
                        await conn.execute(query)
                    ...
                    query = (
                        ...
                        .where(keypair_resource_usages.c.concurrency_used != 0)
                        .where(sa.not_(
                            keypair_resource_usages.c.access_key.in_(concurrency_used_per_key.keys()),
                        ))
                    )
                    await conn.execute(query)
                else:
                            ...
                            async with self.db.begin() as conn:
                                if kernel['cluster_role'] == DEFAULT_ROLE:
                                    ...
                                    await conn.execute(
                                        ...
                                        .values({
                                            'concurrency_used':
                                            keypair_resource_usages.c.concurrency_used - 1,
                                        })
                                        .where(
                                            keypair_resource_usages.c.access_key == kernel['access_key'],
                                        ),
                                    )
                                await conn.execute(
                                    sa.update(kernels)
