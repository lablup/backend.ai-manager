    ...
    for row in rows:
        ...
        c_info = {
            ...
            'disk_used': (int(last_stat['io_scratch_size']['stats.max'])
                          if last_stat else 0),
            'io_read': int(last_stat.get('io_read', {}).get('current', 0)) if last_stat else 0,
            'io_write': int(last_stat.get('io_write', {}).get('current', 0)) if last_stat else 0,
            'used_time': used_time,
            'used_days': used_days,
...
async def get_time_binned_monthly_stats(request, user_uuid=None):
    ...
    while ts < now:
        ...
        while idx < rowcount and \
            ...
            if row.last_stat:
                if _stat := row.last_stat.get('io_read'):
                    io_read_bytes += int(_stat['current'])
                if _stat := row.last_stat.get('io_write'):
                    io_write_bytes += int(_stat['current'])
                if _stat := row.last_stat.get('io_scratch_size'):
