...
from backend.ai.argparse import HostPortPair, host_port_pair, ipaddr, port_no
import configargparse


def load_config(argv=None, extra_args_func=None):
    parser = configargparse.ArgumentParser()
    parser.add('--agent-port',   env_var='BACKEND.AI_AGENT_PORT', type=port_no, default=6001,
               help='The TCP port number where the agent instances are listening on. (default: 6001)')
    parser.add('--redis-addr', env_var='BACKEND.AI_REDIS_ADDR', type=host_port_pair,
               default=HostPortPair(ip_address('127.0.0.1'), 6389),
               help='The hostname-port pair of a redis server. (default: localhost:6379)')
    parser.add('--db-addr', env_var='BACKEND.AI_DB_ADDR', type=host_port_pair,
               default=HostPortPair(ip_address('127.0.0.1'), 5442),
               help='The hostname-port pair of a database server. (default: localhost:5432)')
    parser.add('--db-name', env_var='BACKEND.AI_DB_NAME', type=str, default='backend.ai',
               help='The database name. (default: backend.ai)')
    parser.add('--db-user', env_var='BACKEND.AI_DB_USER', type=str, default='postgres',
               help='The username to authenticate to the database server. (default: postgres)')
    parser.add('--db-password', env_var='BACKEND.AI_DB_PASSWORD', type=str, default='develove',
               help='The password to authenticate to the database server. (default: develove)')
    parser.add('--debug', env_var='BACKEND.AI_DEBUG', action='store_true', default=False,
               help='Set the debug mode and verbose logging. (default: false)')
    parser.add('--kernel-ip-override', env_var='BACKEND.AI_KERNEL_IP_OVERRIDE', type=ipaddr, default=None,
               help='The IP address that overrides the actual IP address of kernel containers '
                    ...
                    'all Backend.Ai components and a dockerized front-end service '
